...
def test_all_populations_should_be_inactive_when_timers_are_positive():
    ...
    assert ([], population.ids.tolist()) == action.active_inactive_ids()
...
def test_active_inactive_ids_should_mark_timer_0_as_active():
    ...
    assert (population.ids[:5].tolist(), population.ids[5:].tolist()) == action.active_inactive_ids()
...
def test_get_activity_should_be_default_by_default():
    ...
    assert [1.] * 10 == action.get_param("activity", population.ids).tolist()
...
def test_get_activity_should_be_aligned_for_each_state():
    ...
    assert [1, 10] == action.get_param("activity", population.ids[-2:]).tolist()
...
def test_scenario_transiting_to_state_with_0_back_to_default_prob_should_remain_there():
    ...
    # here we are saying that some action on populations 5 to 9 is triggering a
...
def test_bugfix_collisions_force_act_next():
    ...
    action.timer_tick(population.ids)
    assert action.timer.loc[forced]["remaining"].tolist() == [0, 0, 0, 0, 0]
    assert action.timer.loc[not_forced]["remaining"].equals(
